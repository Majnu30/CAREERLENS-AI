...
def _make_assessment_token(payload: Optional[Dict[str, Any]] = None) -> str:
    ...
    data.setdefault("iat", int(_now_ist().timestamp()))
...
def _load_public_assessment_record(token: str):
    ...
    try:
        ...
        # `expires_at` is link-retention metadata, not the assessment access window.
        # The actual open/close enforcement is handled by _assessment_window_state().
        # In particular, do NOT reject the record merely because end_at has passed; the
        # candidate must receive the explicit "Assessment Closed" state.
...
def assessment_result(questions: List[Dict], answers: Dict) -> Dict[str, Any]:
    ...
    return {
        ...
        "submitted_at": _now_ist().strftime("%Y-%m-%d %I:%M:%S %p"),
...
def _save_public_assessment_result(owner_user_id: str, assessment: Dict[str, Any], candidate_id: str, result: Dict[str, Any]) -> None:
    """Persist a candidate result in a dedicated table and mirror it into recruiter state.

    The dedicated table is the source of truth for recruiter reporting. This avoids relying
    on Streamlit session state, which is different between the candidate and recruiter browsers.
    """
    # Never silently discard a valid submission. Resolve missing ownership/candidate
    # information from the persisted assessment record when possible.
    token = str(result.get("token", "")).strip()
    if token and (not owner_user_id or not candidate_id):
        try:
            persisted = _load_public_assessment_record(token)
            if persisted and persisted[0] is not None:
                persisted_assessment, persisted_candidate, persisted_candidate_id, persisted_owner = persisted
                owner_user_id = owner_user_id or persisted_owner
                candidate_id = candidate_id or persisted_candidate_id
                if isinstance(assessment, dict) and isinstance(persisted_assessment, dict):
                    assessment = {**persisted_assessment, **assessment}
        except Exception:
            pass
    if not owner_user_id or not candidate_id or not token:
        # This is a genuine invalid submission, not an empty dashboard condition.
        raise ValueError("Assessment submission is missing owner, candidate, or token.")

    result["owner_user_id"] = owner_user_id
    result["candidate_id"] = candidate_id
    result["assessment_id"] = result.get("assessment_id") or assessment.get("id", "")
    result["result_saved"] = True
    submitted_at = str(result.get("submitted_at") or _now_ist().strftime("%Y-%m-%d %I:%M:%S %p"))
    ...
    with conn:
        conn.execute(
            """INSERT INTO assessment_submissions
               (token, owner_user_id, candidate_id, assessment_id, result_json, submitted_at)
               VALUES(?,?,?,?,?,?)
            ...
            (token, owner_user_id, candidate_id, str(result.get("assessment_id", "")), payload, submitted_at),
        )
        # Store the result inside the candidate-specific public assessment too. This makes
        # the submission recoverable even if recruiter_state is stale or the reporting table
        # was created after an older invitation was issued.
        row = conn.execute("SELECT assessment_json FROM public_assessments WHERE token=?", (token,)).fetchone()
        if row:
            try:
                public_record = json.loads(row[0]) if row[0] else {}
                if not isinstance(public_record, dict):
                    public_record = {}
                public_record["result"] = result
                public_record["used"] = True
                public_record["submitted_at"] = submitted_at
                conn.execute(
                    "UPDATE public_assessments SET assessment_json=?, used=1 WHERE token=?",
                    (json.dumps(public_record, ensure_ascii=False), token),
                )
    ...
    # Mirror into the recruiter's persisted workspace so existing UI and candidate pipeline
    # remain compatible with older data.
    data = _db_load_recruiter_state(owner_user_id)
    ...
    candidate["assessment_completed_at"] = submitted_at
    candidate["status"] = "Assessment Completed"

    submissions = [x for x in (data.get("submissions", []) or [])
...
def _load_persisted_assessment_submissions(owner_user_id: str) -> List[Dict[str, Any]]:
    """Load assessment results from every durable assessment source, de-duplicated by token."""
    if not owner_user_id:
        return []
    results_by_token: Dict[str, Dict[str, Any]] = {}
    try:
        conn = get_db_connection()
        # Primary source: dedicated submission table.
        rows = conn.execute(
            "SELECT result_json FROM assessment_submissions WHERE owner_user_id=? ORDER BY submitted_at DESC",
            (owner_user_id,),
        ).fetchall()
        for (raw,) in rows:
            try:
                ...
                if isinstance(item, dict):
                    ...
                    results_by_token[token or uuid.uuid4().hex] = item
            except (TypeError, ValueError, json.JSONDecodeError):
                continue

        # Recovery source: candidate-specific public assessment records. Older or partially
        # migrated submissions may exist here even when the reporting table is empty.
        public_rows = conn.execute(
            "SELECT token, assessment_json FROM public_assessments WHERE owner_user_id=? ORDER BY created_at DESC",
            (owner_user_id,),
        ).fetchall()
        for token, raw in public_rows:
            try:
                ...
                if isinstance(result, dict):
                    ...
                    result.setdefault("assessment_id", record.get("id", ""))
                    results_by_token[str(token)] = result
            except (TypeError, ValueError, json.JSONDecodeError):
                continue
    except sqlite3.Error:
        return list(results_by_token.values())
    return sorted(results_by_token.values(), key=lambda x: str(x.get("submitted_at", "")), reverse=True)
...
def render_public_recruiter_assessment(token: str) -> None:
    ...
    # Never allow an unscheduled public link to fall through to the exam.
    start = _parse_assessment_time(assessment.get("start_at"))
    end = _parse_assessment_time(assessment.get("end_at"))
    if not start or not end or end <= start:
        st.markdown("## ⚠️ Assessment Schedule Unavailable")
        st.error("This assessment does not have a valid opening and closing time. Please contact the recruiter for a new invitation.")
        st.stop()
    ...
    if window_state == "scheduled":
        ...
        st.warning(
            f"Please wait. Your assessment will start at **{start.strftime('%d %b %Y, %I:%M %p')} IST**."
        )
        st.info("The Start Assessment button will appear automatically when the assessment window opens.")
        components.html(
            f"""<div style="font-family:Arial,sans-serif;text-align:center;padding:20px;border:1px solid #dbeafe;border-radius:18px;background:#eff6ff;">
                <div style="font-size:12px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.08em;">Time remaining</div>
                <div id="cl-wait-countdown" style="font-size:34px;font-weight:900;color:#1d4ed8;margin-top:5px;">--:--:--</div>
                <div style="font-size:12px;color:#64748b;margin-top:4px;">India Standard Time (IST)</div>
            </div>
            <script>
            const target = {start.timestamp() * 1000:.0f};
            const el = document.getElementById("cl-wait-countdown");
            function tick() {{
            ...
            </script>""", height=130, scrolling=False,
        )
        st.stop()
    if window_state == "closed":
        st.markdown("## 🔒 Assessment Closed")
        st.error(f"The assessment access window ended at **{end.strftime('%d %b %Y, %I:%M %p')} IST**.")
        st.stop()
    ...
    if not st.session_state[started_key]:
        ...
        if st.button("🚀 Start Assessment", type="primary", use_container_width=True, key=f"start_public_assessment_{token[:10]}"):
            # Server-side recheck prevents a stale browser page from starting early.
            if _assessment_window_state(assessment) != "live":
                st.warning("The assessment window is not currently open. Please wait until the scheduled start time.")
                st.rerun()
...
# ============================================================
    ...
    if st.session_state.get("user_id"):
        try:
            st.session_state.recruiter_data = _db_load_recruiter_state(st.session_state.user_id)
            st.session_state.recruiter_candidates = st.session_state.recruiter_data.get("candidates", [])
            persisted_submissions = _load_persisted_assessment_submissions(st.session_state.user_id)
            # Dedicated submission records are authoritative; legacy recruiter-state submissions
            # are used only as a fallback for older assessments created before this upgrade.
            legacy_submissions = [
                x for x in st.session_state.recruiter_data.get("submissions", [])
                if isinstance(x, dict)
            ]
            merged = {str(x.get("token", i)): x for i, x in enumerate(legacy_submissions)}
            for x in persisted_submissions:
                merged[str(x.get("token"))] = x
            st.session_state.recruiter_assessment_submissions = merged
        except Exception:
            pass
    ...
    if st.session_state.active_tool == "Dashboard":
        ...
        # Always read the latest persisted submissions when the recruiter opens this section.
        if st.session_state.get("user_id"):
            # Force a fresh database read on every render; never trust the old Streamlit
            # session copy for assessment results.
            submissions = _load_persisted_assessment_submissions(st.session_state.user_id)
        ...
        if not submissions:
            ...
            for item in submissions:
                display.append({
                    ...
                    "Status": item.get("assessment_status", item.get("status", "Completed")),
