...
.material-symbols-outlined{
  ...
  vertical-align:middle;
    ...
    :root{
      ...
      --line: #e2e8f0;
      --page: #f8fafc;
      --grad-primary: linear-gradient(135deg, #1d4ed8 0%, #2563eb 52%, #3b82f6 100%);
      --grad-primary-hover: linear-gradient(135deg, #1e40af 0%, #1d4ed8 52%, #2563eb 100%);
      --grad-nav-active: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%);
      --grad-badge: linear-gradient(135deg, #2563eb 0%, #3b82f6 100%);
    ...
    *{ box-sizing: border-box; }
    ...
    html, body, .stApp {
      ...
      background: radial-gradient(circle at 90% 8%, rgba(59, 130, 246, 0.08), transparent 35%),
    ...
    .landing-frame:after { content:""; position:absolute;width:240px;height:240px;left:-150px;bottom:-150px;border-radius:50%;background:rgba(59,130,246,.08); }
    ...
    .landing-brand span { background:linear-gradient(135deg,#1d4ed8,#2563eb,#3b82f6);-webkit-background-clip:text;background-clip:text;color:transparent; }
    ...
    .landing-gradient { background:linear-gradient(135deg,#1d4ed8,#2563eb,#3b82f6);-webkit-background-clip:text;background-clip:text;color:transparent; }
    .landing-copy { max-width:760px; color:#64748b; font-size:clamp(.94rem,1.5vw,1.08rem); line-height:1.75; }
    .semantic-icon { width:42px;height:42px;display:flex;align-items:center;justify-content:center;border-radius:13px;margin-bottom:12px;background:linear-gradient(135deg,#1d4ed8,#2563eb);color:#fff;box-shadow:0 8px 18px rgba(37,99,235,.22);font-size:20px;font-weight:900; }
    ...
    .recruiter-command { display:flex;align-items:center;gap:12px;padding:12px 15px;margin:0 0 18px;border:1px solid #dbeafe;border-radius:16px;background:linear-gradient(135deg,#f8fbff,#f4f8ff);box-shadow:0 6px 18px rgba(15,23,42,.04); }
    .recruiter-command-icon { width:40px;height:40px;border-radius:12px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#2563eb,#3b82f6);color:#fff;font-size:19px; }
    ...
    .landing-icon { width:42px;height:42px;display:flex;align-items:center;justify-content:center;border-radius:13px;margin-bottom:12px;color:white;font-weight:900;background:linear-gradient(135deg,#1d4ed8,#2563eb);box-shadow:0 8px 18px rgba(37,99,235,.22); }
    .landing-feature-title { font-weight:900;color:#0f172a; }
    .landing-feature-text { margin-top:4px;color:#64748b;font-size:.82rem; }
    .landing-access { margin-top:22px; padding:22px; border-radius:22px; background:linear-gradient(135deg,rgba(239,246,255,.96),rgba(239,246,255,.72)); border:1px solid #dbeafe; }
    ...
    .landing-divider { height:3px;border-radius:99px;background:linear-gradient(90deg,#1d4ed8,#2563eb,#3b82f6);margin:0 0 18px; }
    ...
    .tag-purple { background: #eff6ff; color: #1d4ed8; border: 1px solid #dbeafe; }
    ...
    .access-title span { color: #2563eb; }
...
/* ============================================================
   CAREERLENS AI — VISUAL CONSISTENCY OVERRIDE
   Goal 1: preserve functionality; normalize the visual system.
   ============================================================ */
:root{
  --grad-primary: linear-gradient(135deg,#1d4ed8 0%,#2563eb 52%,#3b82f6 100%);
  --grad-primary-hover: linear-gradient(135deg,#1e40af 0%,#1d4ed8 52%,#2563eb 100%);
...
.stApp{
  background:
    radial-gradient(circle at 90% 8%,rgba(59,130,246,.08),transparent 35%),
    linear-gradient(180deg,#fafcff 0%,#f1f5f9 100%) !important;
}

.stApp p,.stApp span,.stApp label,.stApp div,.stApp input,.stApp textarea,
.stApp button,.stApp select{
  font-family:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif !important;
}
...
/* White cards */
.content-box,.gateway-card,.kpi-card,.tool-box-card,.header-banner{
  background:#fff !important;
  border-color:#e2e8f0 !important;
}

/* Blue brand treatment */
.brand-accent,.landing-gradient{
...
if st.session_state.active_workspace == "Job Seeker Workspace":
    ...
    st.markdown(
        ...
        <div class="kpi-grid">
            ...
            <div class="kpi-card">
                <div class="kpi-icon-badge" style="background:#faf5ff; color:#2563eb;"><span class="material-symbols-outlined">monitoring</span></div>
                <div>
    ...
    if st.session_state.active_tool == "Dashboard":
        ...
        with c1:
            ...
            if st.button("Resume Intelligence", icon=":material/description:", key="card_c1_btn", use_container_width=True):
                ...
                st.rerun()
        with c2:
            st.markdown("""<div class="tool-box-card"><div class="tool-icon-circle" style="background:#faf5ff; color:#2563eb;"><span class="material-symbols-outlined">assignment</span></div><div class="tool-title">Pre-Interview Exam</div><div class="tool-desc">Standardized MCQ domain qualifying assessment.</div></div>""", unsafe_allow_html=True)
