...
except ImportError:  # Keep non-email API routes usable when the optional mail module is absent.
    def send_html_email(to_email: str, subject: str, content: str):
        return False, "Email service is not installed/configured on the server."
...
class MockInterviewRequest(BaseModel):
    ...
    transcript: List[Dict[str, Any]] = Field(default_factory=list, max_length=50)
...
def _resume_analysis(text: str, filename: str, target_role: str = "") -> Dict[str, Any]:
    ...
    return {
        ...
        "extracted_text":text,"source":"api-analysis-v5","analysis_version":5,"target_role":target_role,
    }
...
def _match(resume_text: str, job_description: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "source":"local-api-v5","analysis_version":5,
    }
...
class AssessmentGenerateRequest(BaseModel):
    ...
    seed: str = Field("", max_length=300)


def _api_assessment_bank(role: str) -> List[Dict[str, Any]]:
    role_l = role.lower()
    if any(x in role_l for x in ("software", "developer", "programmer", "engineer", "devops", "cloud")):
        items = [
            ...
            ("Why are code reviews valuable?", ["They catch defects and improve maintainability through shared knowledge", "They replace all testing", "They guarantee zero bugs", "They remove documentation"], 0),
        ]
    elif any(x in role_l for x in ("data", "analytics")):
        items = [
            ("What should you do first when an analysis produces a surprising result?", ["Validate data quality and assumptions", "Publish immediately", "Delete outliers without review", "Change chart colors"], 0),
    ...
    return [{"id": i + 1, "question": q, "options": opts, "answer": opts[idx]} for i, (q, opts, idx) in enumerate(items)]


@app.post("/api/assessment/generate")
def generate_assessment(payload: AssessmentGenerateRequest) -> Dict[str, Any]:
    """Generate a role-based assessment bank for integrations and testing.

    The production public candidate attempt is persisted by Streamlit per candidate,
    where question and option order are randomized independently.
    """
    bank = _api_assessment_bank(payload.role.strip())
    rng = __import__("random").Random(payload.seed or os.urandom(8).hex())
    rng.shuffle(bank)
    selected = []
    target = max(10, min(50, payload.question_count))
    for i in range(target):
        ...
        selected.append({
            "id": i + 1,
            "question": base["question"] + (f" (Scenario {i // len(bank) + 1})" if i >= len(bank) else ""),
            "options": options,
            "answer": base["answer"],
        })
    return {
        ...
        "questions": selected,
        "randomized": True,
        "source": "careerlens-api-assessment-v2",
        "analysis_version": 6,
    }
...
def _interview_questions(role: str, resume_text: str = "", company: str = "") -> List[str]:
    role = role.strip()
    company = company.strip() or "this company"
    lower = resume_text.lower()
    detected = sorted(_skill_set(resume_text))
    questions = [
        f"Please introduce yourself and walk me through your background, focusing on why you are a good fit for the {role} role.",
        f"Why did you choose the {role} career path, and what interests you about this opportunity?",
        f"Why do you want to work at {company}, and what value would you bring to the team?",
        "Which achievement on your resume are you most proud of, and what was your personal contribution?",
    ]
    for skill in detected[:10]:
        questions.append(f"Your resume mentions {skill}. Tell me about a real situation where you used it and what the outcome was.")
    # Use resume phrases as evidence prompts without pretending the API understands unseen details.
    if resume_text:
        sentences = [x.strip() for x in re.split(r"[.!?\\n]", resume_text) if len(x.strip()) > 25]
        for sentence in sentences[:5]:
            questions.append(f"I noticed this experience in your resume: '{sentence[:180]}'. What exactly did you do, what challenge did you face, and what was the result?")
    questions.extend([
        ...
        "Describe a disagreement with a teammate or stakeholder and how you handled it.",
        "How do you prioritize when several tasks are urgent?",
        f"What is one skill you still need to strengthen to become excellent at {role}?",
        "What would you aim to accomplish in your first 90 days?",
        "Do you have any questions you would ask the interviewer?",
    ])
    return list(dict.fromkeys(questions))


def _score_interview(transcript: List[Dict[str, Any]], role: str) -> Dict[str, Any]:
    if not transcript:
        return {"overall": 0, "communication": 0, "completeness": 0, "evidence": 0, "relevance": 0, "answered": 0}
    answers = [str(x.get("answer", "")) for x in transcript]
    lengths = [len(a.split()) for a in answers]
    completeness = round(sum(1 for n in lengths if n >= 30) / len(answers) * 100)
    evidence_terms = ("because", "example", "result", "impact", "metric", "improved", "built", "developed", "implemented", "learned")
    evidence = round(sum(1 for a in answers if any(k in a.lower() for k in evidence_terms)) / len(answers) * 100)
    relevance = round(sum(1 for a in answers if role.lower() in a.lower() or any(k in a.lower() for k in ("project", "experience", "skill", "role"))) / len(answers) * 100)
    detail = min(100, round(sum(lengths) / len(lengths) * 2.2))
    communication = min(100, round(detail * .65 + evidence * .2 + relevance * .15))
    overall = max(5, min(98, round(communication*.45 + completeness*.25 + evidence*.15 + relevance*.15)))
    return {"overall": overall, "communication": communication, "completeness": completeness, "evidence": evidence, "relevance": relevance, "answered": len(transcript)}


@app.post("/api/mock-interview/questions")
def mock_interview_questions(payload: MockInterviewRequest) -> Dict[str, Any]:
    questions = _interview_questions(payload.role, payload.resume_text, payload.company)
    used = {str(x.get("question", "")) for x in payload.transcript}
    remaining = [q for q in questions if q not in used]
    next_question = remaining[0] if remaining else f"As a {payload.role}, describe a realistic high-priority challenge and explain how you would solve it."
    return {"next_question": next_question, "available_questions": len(remaining), "source": "resume-aware-local-v1", "analysis_version": 5}


@app.post("/api/mock-interview/score")
def mock_interview_score(payload: MockInterviewRequest) -> Dict[str, Any]:
    return {**_score_interview(payload.transcript, payload.role), "source": "local-interview-score-v1", "analysis_version": 5}
...
def career_roadmap(payload: RoadmapRequest) -> Dict[str, Any]:
    ...
    return {
        ...
        ],"current_skills":skills,"missing_skills":missing[:10],"target_role":payload.target_role,"source":"local-api-v5","analysis_version":5
    }
...
def score_assessment(payload: AssessmentScoreRequest) -> Dict[str, Any]:
    """Score a submitted MCQ assessment consistently for recruiter reporting."""
    correct = 0
    unanswered = 0
    wrong = 0
    for question in payload.questions:
        qid = str(question.get("id", ""))
        selected = payload.answers.get(qid, payload.answers.get(question.get("id")))
        expected = question.get("answer")
        if selected in (None, ""):
            unanswered += 1
        elif str(selected) == str(expected):
            correct += 1
        else:
            wrong += 1
...
@app.post("/api/assessment/score")
def assessment_score(payload: AssessmentScoreRequest) -> Dict[str, Any]:
    """Score a completed role-based assessment consistently for external clients."""
    ...
    for q in payload.questions:
        ...
        if selected is None or selected == "":
            ...
            wrong += 1
    total = len(payload.questions)
    return {"score": correct, "total": total, "percentage": round(correct / total * 100, 1) if total else 0, "correct_count": correct, "wrong_count": wrong, "unanswered_count": unanswered}
