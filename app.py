...
def _assessment_public_url(token: str) -> str:
    # Assessment links are ALWAYS public CareerLens links. Never fall back to a local development URL.
    base = "https://career-lens-ai.streamlit.app"
...
def _find_recruiter_assessment_by_token(token: str):
    """Find an assessment across persisted recruiter accounts, not only the current browser session."""
    token = str(token or "").strip()
    if not token:
        return None, None, None, None

    # First check the current session for speed.
    data = st.session_state.get("recruiter_data") or {}
    candidates = st.session_state.get("recruiter_candidates") or data.get("candidates", [])
    for assessment in data.get("assessments", []):
        ...
        for candidate_id, candidate_token in tokens.items():
            if str(candidate_token) == token:
                ...
                if candidate:
                    return assessment, candidate, candidate_id, st.session_state.get("user_id", "")

    # Public candidate links are opened without recruiter authentication. Search the
    # persistent recruiter_state table so the link works on any device/browser.
    try:
        conn = get_db_connection()
...
def _save_public_assessment_result(owner_user_id: str, assessment: Dict[str, Any], candidate_id: str, result: Dict[str, Any]) -> None:
    """Persist candidate assessment results to the recruiter's durable state."""
...
def render_public_recruiter_assessment(token: str) -> None:
    ...
    st.info("You are completing an assessment for the company and recruiter shown above. Your submitted score will be returned to the recruiting workspace.")
...
# ============================================================
    ...
    if st.session_state.active_tool == "Dashboard":
        ...
        company_name = st.text_input("Company / Organization", value=campaign.get("company", ""), placeholder="e.g. TCS, Microsoft, Deloitte", key="campaign_company")
        recruiter_name = st.text_input("Recruiter Name", value=campaign.get("recruiter_name", st.session_state.get("username", "Recruiter")), key="campaign_recruiter_name")
        recruiter_email = st.text_input("Recruiter Contact Email", value=campaign.get("recruiter_email", st.session_state.get("username", "") if "@" in st.session_state.get("username", "") else ""), key="campaign_recruiter_email")
        ...
        if st.button("💾 Save Campaign", type="primary", use_container_width=True, key="save_campaign"):
            data["campaign"] = {"role": role, "company": company_name.strip(), "recruiter_name": recruiter_name.strip(), "recruiter_email": recruiter_email.strip(), "job_description": job_description.strip()}
            persist_recruiter()
        ...
        st.markdown(f"**Company:** {html.escape(company_target)}  ·  **Recruiter:** {html.escape(recruiter_target)}" + (f"  ·  **Contact:** {html.escape(recruiter_email_target)}" if recruiter_email_target else ""), unsafe_allow_html=True)
