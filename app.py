...
st.markdown(
    ...
    :root{
      --ink:#101b3a; --muted:#66738f; --line:#e5eaf5; --page:#f7f9fe;
      --blue:#2864f0; --violet:#7654ee; --lav:#efeaff; --white:#fff;
      --grad:linear-gradient(135deg,#2d67f2 0%,#7554ec 100%);
      --shadow:0 18px 55px rgba(35,55,105,.10);
    }
    *{box-sizing:border-box}
    html,body,.stApp{font-family:'Plus Jakarta Sans',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif!important;background:var(--page)!important;color:var(--ink)!important}
    .stApp{background:radial-gradient(circle at 85% 10%,rgba(132,107,246,.10),transparent 28%),linear-gradient(180deg,#fbfcff 0%,#f6f8fd 100%)!important}
    .block-container{max-width:1500px!important;padding:22px 30px 48px!important}
    ...
    [data-testid="stSidebar"] .stButton>button{background:transparent!important;color:#53617c!important;border:0!important;box-shadow:none!important;border-radius:9px!important;text-align:left!important;justify-content:flex-start!important;padding:9px 11px!important;font-size:.79rem!important;font-weight:600!important;min-height:36px!important}
    [data-testid="stSidebar"] .stButton>button:hover{background:#f2f5ff!important;color:#2c5ce7!important;transform:none!important}
    [data-testid="stSidebar"] .stButton>button[kind="primary"]{background:#eaf0ff!important;color:#2459df!important;border:0!important;box-shadow:none!important}
    [data-testid="stSidebar"] .stButton>button[kind="primary"] *{color:#2459df!important;-webkit-text-fill-color:#2459df!important}
    .sidebar-brand-box{display:flex;align-items:center;gap:9px;padding:4px 5px 17px;border-bottom:1px solid #edf0f6;margin-bottom:14px}
    .sidebar-brand-box>div:first-child{font-size:22px!important}
    .sidebar-user-box{background:#f7f9ff;border:1px solid #e7ebf6;border-radius:12px;padding:10px 11px;margin-bottom:13px}
    ...
    .tag-badge{display:inline-flex;align-items:center;padding:4px 9px;border-radius:999px;font-size:.66rem;font-weight:700;letter-spacing:.01em}
    .tag-blue{background:#eef4ff;color:#2d60d9;border:1px solid #dbe7ff}.tag-purple{background:#f5efff;color:#7b50d9;border:1px solid #eadcff}.tag-green{background:#edf9f3;color:#14825a;border:1px solid #d7f0e4}.tag-amber{background:#fff7e9;color:#a66b09;border:1px solid #f7e8c7}
    .header-banner{background:#fff;border:1px solid var(--line);border-radius:16px;padding:18px 22px;margin-bottom:20px;box-shadow:0 7px 24px rgba(35,55,105,.04);display:flex;justify-content:space-between;align-items:center}
        ...
        <div class="hero-shell">
          ...
          <div class="hero-grid">
            <div>
              ...
              <p class="hero-copy">Get personalized insights, intelligent recommendations and the tools you need to achieve your career goals.</p>
          ...
          <div class="hero-stats">
            <div class="hero-stat"><div class="hero-stat-icon">✓</div><div><b>10K+</b><span>Happy Users</span></div></div>
...
if not st.session_state.selected_gateway:
    st.markdown(
        ...
        <div class="header-banner">
          <div><div class="header-title">Welcome, {html.escape(st.session_state.username)}! 👋</div><div class="header-sub">Choose the workspace that matches what you want to accomplish.</div></div>
...
st.markdown(
    f"""
    <div style="background:#fff;border-bottom:1px solid #edf0f6;margin:-4px 0 18px;padding:2px 2px 15px;display:flex;align-items:center;justify-content:space-between">
      <div><div style="font-weight:800;font-size:.96rem;color:#172341">CareerLens <span style="color:#6756ed">AI</span></div><div style="font-size:.66rem;color:#8490a6">{workspace_label} workspace · intelligent career decisions</div></div>
      <div style="display:flex;align-items:center;gap:9px"><span style="font-size:.72rem;color:#66738f">{html.escape(st.session_state.username)}</span><span style="width:30px;height:30px;border-radius:50%;background:linear-gradient(135deg,#e8efff,#efe7ff);display:flex;align-items:center;justify-content:center;color:#4d5bd7;font-weight:800">●</span></div>
    </div>
    """, unsafe_allow_html=True)
