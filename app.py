...
def _save_public_assessment_record(owner_user_id: str, candidate_id: str, assessment: Dict[str, Any], token: str) -> None:
    ...
    expires_at = str(assessment.get("end_at", "") or "").strip() or (datetime.now() + timedelta(days=30)).isoformat(timespec="seconds")
    with conn:
        conn.execute(
...
# ============================================================
    ...
    if st.session_state.active_tool == "Dashboard":
        ...
        w1, w2 = st.columns(2)
        open_date = w1.date_input("Open date", value=datetime.now().date(), key="assessment_open_date")
        open_time = w1.time_input("Open time", value=datetime.now().replace(second=0, microsecond=0).time(), key="assessment_open_time")
        close_date = w2.date_input("Close date", value=datetime.now().date(), key="assessment_close_date")
        close_time = w2.time_input("Close time", value=(datetime.now()+timedelta(hours=2)).replace(second=0, microsecond=0).time(), key="assessment_close_time")
        start_at = datetime.combine(open_date, open_time).isoformat(timespec="seconds")
        end_at = datetime.combine(close_date, close_time).isoformat(timespec="seconds")
        valid_window = datetime.fromisoformat(end_at) > datetime.fromisoformat(start_at)
        if not valid_window:
