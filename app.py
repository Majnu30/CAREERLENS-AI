...
def api_analyze_resume(file, target_role: str = "") -> Dict[str, Any]:
    ...
    try:
        ...
        if res.ok and isinstance(res.json(), dict):
            ...
            if int(data.get("analysis_version", 0) or 0) >= 4 and data.get("score_breakdown"):
...
def api_match_job(resume_text: str, job_description: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if res.ok and isinstance(res.json(), dict) and int(res.json().get("analysis_version", 0) or 0) >= 4:
...
def generate_mock_interview_questions(role: str, count: int) -> List[str]:
    role_l = role.lower()
    domain = "technical" if any(x in role_l for x in ("engineer", "developer", "data", "scientist", "developer", "analyst", "cyber", "cloud", "devops", "qa")) else "professional"
    specific = []
    if "ai" in role_l or "machine learning" in role_l:
        specific = [
            "Explain a machine-learning project you built, including the data, model choice and evaluation.",
            "How would you detect and reduce overfitting in a production ML model?",
            "How would you monitor an ML model after deployment?",
            "How would you handle missing, noisy or biased training data?",
            "Explain a trade-off you would make between model accuracy, latency and cost.",
        ]
    elif "data" in role_l or "analyst" in role_l:
        specific = [
            "Walk through a data analysis project from raw data to business recommendation.",
            "How do you validate a dataset before drawing conclusions?",
            "Describe a dashboard or report you built and the decision it supported.",
            "How would you investigate an unexpected change in a KPI?",
            "Explain how you communicate uncertainty in an analysis.",
        ]
    elif "hr" in role_l or "human resource" in role_l:
        specific = [
            "How would you improve a hiring process with measurable outcomes?",
            "How do you handle a sensitive employee-relations situation?",
            "Which metrics would you use to evaluate recruitment quality?",
            "How would you improve candidate experience while controlling hiring cost?",
        ]
    elif "marketing" in role_l:
        specific = [
            "How would you design a campaign for a new product with a limited budget?",
            "Which metrics would tell you whether a campaign is working?",
            "Describe how you would investigate a drop in conversion rate.",
            "How do you balance creative decisions with data?",
        ]
    elif "finance" in role_l or "account" in role_l:
        specific = [
            "How would you validate financial data before preparing a report?",
            "Describe a financial-analysis decision you would support with evidence.",
            "How would you investigate an unexpected variance in a budget?",
            "How do you communicate financial risk to a non-finance stakeholder?",
        ]
    elif "project manager" in role_l or "product manager" in role_l:
        specific = [
            "How would you prioritize competing requirements from stakeholders?",
            "Describe how you would manage a project that is falling behind schedule.",
            "Which metrics would you use to measure product or project success?",
            "How would you handle disagreement between two stakeholders?",
        ]
    common = [
        f"Tell me about yourself and why you are targeting the {role} role.",
        f"What are the most important skills for a successful {role}, and how have you demonstrated them?",
        "Describe a difficult problem you solved. Explain your reasoning and measurable outcome.",
        "Tell me about a mistake you made at work or in a project and what you changed afterward.",
        "Describe a disagreement with a teammate and how you resolved it.",
        "Tell me about a project where you took ownership beyond the original requirement.",
        "How do you learn a new tool, technology or process when you have limited guidance?",
    ...
    if domain == "technical":
        common += [
            f"How would you troubleshoot a production issue relevant to {role}?",
            "How do you balance correctness, maintainability and delivery speed?",
            "Describe how you would test a change before releasing it.",
        ]
    bank = specific + common
    return [bank[i % len(bank)] if i < len(bank) else f"Scenario {i + 1}: As a {role}, describe how you would handle a realistic high-priority challenge and explain your reasoning." for i in range(max(10, min(50, count)))]


def score_mock_interview(transcript: List[Dict[str, Any]], role: str) -> Dict[str, Any]:
    if not transcript:
        return {"overall": 0, "communication": 0, "completeness": 0, "strengths": [], "improvements": ["Provide answers to the interview questions."]}
    ...
    keyword_evidence=sum(1 for x in transcript if any(k in str(x.get("answer", "")).lower() for k in ("because", "result", "impact", "example", "metric", "improved", "learned")))
    communication=min(100, round(avg * .65 + keyword_evidence / len(transcript) * 35))
    overall=max(5, min(98, round(communication * .55 + completeness * .45)))
    strengths=[f"Completed {len(transcript)} role-specific interview responses."]
    if avg >= 60: strengths.append("Responses contain useful detail and context.")
    if keyword_evidence >= max(1, len(transcript)//4): strengths.append("Several answers include evidence, outcomes or reasoning.")
    improvements=[]
    if completeness < 70: improvements.append("Give fuller answers with context, actions and results.")
    if keyword_evidence < max(1, len(transcript)//4): improvements.append("Use concrete examples, metrics and outcomes instead of general statements.")
    if not improvements: improvements.append("Keep practicing concise, evidence-based answers tailored to the target role.")
    return {"overall": overall, "communication": communication, "completeness": completeness, "strengths": strengths, "improvements": improvements}
...
if st.session_state.active_workspace == "Job Seeker Workspace":
    ...
    # 3. AI MOCK INTERVIEW
        ...
        if not st.session_state.interview_active and not st.session_state.interview_completed:
            target_interview_role = st.text_input("Search / Enter Any Job Role", placeholder="e.g. AI Engineer, Product Manager, Accountant", key="mock_role_input")
            interview_len = st.select_slider("Interview Questions", options=list(range(10, 51, 5)), value=20, key="mock_count_select")
            if st.button("🚀 Start Mock Interview", use_container_width=True, type="primary", key="start_mock_new"):
                ...
                if len(role) < 2:
                    ...
                    st.session_state.interview_questions = generate_mock_interview_questions(role, interview_len)
            ...
            st.markdown(f"<div class='content-box' style='text-align:center;'><h2>Interview Readiness: <span style='color:#2563eb;'>{rep.get('overall', 0)}%</span></h2><p>{_escape(st.session_state.interview_role)}</p></div>", unsafe_allow_html=True)
            a,b,c = st.columns(3)
            a.metric("Overall", f"{rep.get('overall',0)}%")
            b.metric("Communication", f"{rep.get('communication',0)}%")
            c.metric("Completeness", f"{rep.get('completeness',0)}%")
    ...
    # 4. AI JOB MATCH
        ...
        if st.session_state.job_match_result:
            ...
            st.metric("Job Match Score", f"{m.get('overall', 0)}%")
            st.write(m.get("summary", ""))
            a,b = st.columns(2)
            with a:
                st.markdown("#### Matched Skills")
                for skill in m.get("matched", []): st.success(str(skill))
                if not m.get("matched"): st.info("No explicit matching skills were detected.")
            with b:
                st.markdown("#### Skills to Upgrade")
                for skill in m.get("recommended_skills", m.get("missing", [])):
                    st.warning(f"Upgrade: {skill}")
                if not m.get("missing") and not m.get("recommended_skills"): st.success("No major skill gaps detected from the extracted skill vocabulary.")
            st.caption(f"Experience alignment: {m.get('experience_alignment', 'Unavailable')}")
    ...
    # 8. RESUME BUILDER
        ...
        if st.button("← Back to Dashboard", key="btn_back_bld"):
            st.session_state.active_tool = "Dashboard"
            st.rerun()
        st.markdown("### 📄 Live Resume Builder")
        templates = ["Executive", "Minimal", "Modern Blue", "Modern Purple", "Emerald", "Professional", "Tech", "ATS Classic", "Classic Serif", "Corporate", "Clean Grid", "Modern ATS"]
        template = st.selectbox("Choose Resume Template", templates, key="resume_template_select")
        ...
        with left:
            ...
            resume_data = {"name":rb_name,"email":rb_email,"phone":rb_phone,"location":rb_location,"linkedin":rb_linkedin,"github":rb_github,"headline":rb_headline,"summary":rb_summary,"experience":rb_experience,"education":rb_education,"projects":rb_projects,"skills":rb_skills,"certifications":rb_cert,"achievements":rb_ach}
            ...
            if st.button("⬇️ Generate & Download PDF", use_container_width=True, type="primary", key="download_live_resume"):
                ...
                st.download_button("Download Resume PDF", data=pdf_bytes, file_name="CareerLens_Resume.pdf", mime="application/pdf", use_container_width=True, key="download_resume_file")
        with right:
            st.markdown("#### Live Preview")
            contact = " · ".join([x for x in (rb_email, rb_phone, rb_location, rb_linkedin, rb_github) if x])
            skills_html = "".join(f"<span style='display:inline-block;margin:2px;padding:5px 8px;border-radius:12px;background:#eef2ff;color:#3730a3;font-size:12px;'>{_escape(x.strip())}</span>" for x in rb_skills.split(",") if x.strip())
            preview = f"""<div style='background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:28px;min-height:760px;box-shadow:0 8px 24px rgba(15,23,42,.06);'><h1 style='margin:0;color:#1d4ed8;'>{_escape(rb_name or 'Your Name')}</h1><div style='color:#475569;margin:5px 0 12px;'>{_escape(rb_headline)}</div><div style='font-size:12px;color:#64748b;border-bottom:1px solid #e2e8f0;padding-bottom:12px;'>{_escape(contact)}</div>"""
            if rb_summary: preview += f"<h3>PROFILE</h3><p>{_escape(rb_summary).replace(chr(10),'<br>')}</p>"
            for title, value in (("EXPERIENCE",rb_experience),("EDUCATION",rb_education),("PROJECTS",rb_projects),("SKILLS",skills_html),("CERTIFICATIONS",rb_cert),("ACHIEVEMENTS",rb_ach)):
                if value:
                    body = value if title == "SKILLS" else _escape(value).replace(chr(10),"<br>")
                    preview += f"<h3 style='color:#1d4ed8;border-bottom:1px solid #e2e8f0;padding-bottom:4px;'>{title}</h3><div>{body}</div>"
            preview += "</div>"
