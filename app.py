...
# ============================================================
    ...
    st.markdown(
        """
        <section class="hero">
            <div class="kicker">STANDARDIZED QUALIFYING PORTAL</div>
            <h1>Candidate Assessment.<br><span>Technical & Problem Solving Examination.</span></h1>
            <p>
                Complete your evaluation paper. Responses are automatically submitted to the hiring team.
            </p>
        </section>
        """,
        unsafe_allow_html=True,
    ...
    if not st.session_state.exam_active and not st.session_state.exam_submitted:
        st.markdown("### ⚙️ Examination Entry")
        
        col_ce1, col_ce2 = st.columns(2)
        with col_ce1:
            candidate_identifier = st.text_input("Enter Your Registered Email / Candidate ID:", value=st.session_state.get("exam_candidate_email", ""), placeholder="e.g. rahul@gmail.com")
        with col_ce2:
        ...
        st.markdown(
            """
            <div class="panel">
                <h4 style="margin:0;color:#38bdf8;">📋 Assessment Guidelines:</h4>
                <p style="margin:6px 0 0;color:#cbd5e1;">
                    • 50 questions spanning Aptitude, Core Technical Domain, and System Reliability.<br>
                    • No answers are pre-selected. Click to highlight your answer in solid white.<br>
                    • Submissions are forwarded directly to the recruitment team dashboard.
                </p>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        if st.button("🚀 Begin Assessment", use_container_width=True, key="btn_start_exam_cand"):
            if not candidate_identifier.strip():
                st.warning("Please enter your email or candidate ID to start.")
            else:
                attempt_id = f"{candidate_identifier.strip()}-{uuid.uuid4().hex[:6]}"
                with st.spinner("Loading examination paper..."):
                    try:
                        ...
                        st.session_state.exam_candidate_email = candidate_identifier.strip()
        ...
        answered_count = len([v for v in st.session_state.exam_answers.values() if str(v).strip()])

        st.markdown(f"### 📝 {st.session_state.exam_role}")
        st.caption(f"Candidate: {st.session_state.exam_candidate_email} • {answered_count}/{len(questions)} answered")
        ...
        for section_name in section_order:
            ...
            st.markdown(
                f"""
                <div class="panel" style="margin-top:20px; border-color:rgba(56,189,248,.35);">
                    <div style="font-size:.75rem; color:#38bdf8; font-weight:900; letter-spacing:1.5px;">EXAMINATION SECTION</div>
                    <div style="font-size:1.2rem; color:#f4f7fb; font-weight:900; margin-top:4px;">{section_name}</div>
                    <div style="font-size:.82rem; color:#94a3b8; margin-top:4px;">{len(section_questions)} questions</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
            ...
            for q in section_questions:
                ...
                st.markdown(
                    ...
                    <div style="margin-top:16px; margin-bottom:8px;">
                        ...
                        <div style="font-size:1.02rem; font-weight:750; color:#f4f7fb; margin-top:6px; line-height:1.5;">
                            {q['question']}
                        </div>
                    </div>
                    """,
                ...
                for opt_idx, option in enumerate(q["options"]):
                    ...
                    with option_cols[opt_idx % 2]:
                        if st.button(
                            f"{marker}  {option}",
                            key=f"cand_exam_{st.session_state.exam_attempt_id}_{qid}_{opt_idx}",
                            type=btn_type,
                ...
                if current_answer:
                    if st.button("Clear this selection", key=f"clear_cand_{st.session_state.exam_attempt_id}_{qid}"):
        ...
        if unanswered > 0:
            st.warning(f"⚠️ {unanswered} question(s) currently unattempted.")
        else:
            st.success("✅ All 50 questions answered.")

        if st.button("🏁 Submit Final Assessment", use_container_width=True, key="btn_submit_exam_cand"):
            correct_count = 0
            detailed_items = []
            ...
            for q in questions:
                ...
                status_label = "Correct" if is_correct else ("Unanswered" if not user_ans else "Incorrect")
                detailed_items.append({
                    "id": qid,
                    "section": q.get("section", "General"),
                    "question": q["question"],
                    "candidate_answer": user_ans if user_ans else "(No answer)",
            ...
            # Store result in Recruiter's backend repository
            record_id = str(uuid.uuid4())[:8]
            eval_record = {
                "id": record_id,
                "candidate_email": st.session_state.exam_candidate_email,
                "role": st.session_state.exam_role,
                "score": correct_count,
                "total": total_q,
                "percentage": pct,
                "correct": correct_count,
                "incorrect": total_q - correct_count - unanswered,
                "unanswered": unanswered,
                "submitted_at": datetime.now().strftime("%Y-%m-%d %H:%M"),
                "details": detailed_items
            }
            st.session_state.recruiter_assessments[st.session_state.exam_candidate_email.lower()] = eval_record

            # Update candidate pipeline status if candidate exists in recruiter list
            for c in st.session_state.recruiter_candidates:
                if c.get("email", "").lower() == st.session_state.exam_candidate_email.lower():
                    c["stage"] = "ASSESSMENT COMPLETED"
                    c["assessment_score"] = pct

            st.session_state.exam_active = False
            st.session_state.exam_submitted = True
            log_event("CANDIDATE_ASSESSMENT_SUBMITTED", st.session_state.exam_candidate_email, "N/A", f"Role: {st.session_state.exam_role}")
            st.rerun()
        ...
        st.markdown(
            ...
            <div class="panel" style="text-align:center; padding: 40px 20px;">
                <span class="tag-bubble tag-emerald" style="font-size: 1.1rem; padding: 8px 20px;">✓ ASSESSMENT SUBMITTED</span>
                <h2 style="color: #f4f7fb; margin: 16px 0 8px 0;">Assessment Completed Successfully</h2>
                <p style="color: #94a3b8; font-size: 0.95rem; max-width: 600px; margin: 0 auto;">
                    Your answers have been securely recorded and dispatched to the recruiter. 
                    The recruitment team will review your evaluation and reach out regarding next interview steps.
                </p>
            </div>
            """,
            unsafe_allow_html=True,
        )
        if st.button("⬅️ Return to Main Dashboard", use_container_width=True):
            st.session_state.exam_active = False
            st.session_state.exam_submitted = False
...
# ============================================================
    ...
    st.markdown(
        """
        <section class="hero">
            <div class="kicker">ENTERPRISE RECRUITMENT SUITE</div>
            <h1>Recruiter Workspace.<br><span>End-to-End Pipeline & Assessment Control.</span></h1>
            <p>Launch hiring campaigns, bulk screen resumes with AI, deploy role-specific assessments, and manage interview pipelines.</p>
        </section>
        """,
        unsafe_allow_html=True,
    )
    ...
    assessments_done = len(st.session_state.recruiter_assessments)

    st.markdown("### 📊 Pipeline Overview")
    col_m1, col_m2, col_m3, col_m4 = st.columns(4)
    with col_m1:
        st.markdown(f"""<div class="gauge-box"><div class="gauge-label">Active Roles</div><div style="font-size:2.2rem;font-weight:900;color:#38bdf8;margin:4px 0;">{active_campaigns_count}</div><span class="tag-bubble tag-cyan">Campaigns</span></div>""", unsafe_allow_html=True)
    with col_m2:
        st.markdown(f"""<div class="gauge-box"><div class="gauge-label">Resumes Analyzed</div><div style="font-size:2.2rem;font-weight:900;color:#818cf8;margin:4px 0;">{total_resumes_count}</div><span class="tag-bubble tag-purple">Cohort Size</span></div>""", unsafe_allow_html=True)
    with col_m3:
        st.markdown(f"""<div class="gauge-box"><div class="gauge-label">Shortlisted</div><div style="font-size:2.2rem;font-weight:900;color:#4ade80;margin:4px 0;">{shortlisted_count}</div><span class="tag-bubble tag-emerald">Top Talent</span></div>""", unsafe_allow_html=True)
    with col_m4:
        st.markdown(f"""<div class="gauge-box"><div class="gauge-label">Assessments Done</div><div style="font-size:2.2rem;font-weight:900;color:#fbbf24;margin:4px 0;">{assessments_done}</div><span class="tag-bubble tag-amber">Evaluations</span></div>""", unsafe_allow_html=True)
    ...
    rec_tabs = st.tabs([
        ...
        "🏆 3. Shortlisted Talent",
        "📝 4. Create Role Assessment",
        "📧 5. Dispatch Assessment Links",
        "📊 6. Assessment Results & Review",
        "🏅 7. Candidate Pipeline & Interviews"
    ])

    # 1. Start With a Hiring Campaign
    with rec_tabs[0]:
        st.subheader("🎯 Start a Hiring Campaign")
        st.caption("Define the exact position specifications. This profile drives AI resume screening and dynamic question generation.")

        c_col1, c_col2 = st.columns(2)
        with c_col1:
            camp_role = st.selectbox(
                ...
                key="camp_role_select"
            )
            if camp_role == "Custom Role":
                camp_role_custom = st.text_input("Enter Custom Job Title:", "Mobile Flutter Engineer")
                final_camp_role = camp_role_custom
            else:
                final_camp_role = camp_role

            camp_title = st.text_input("Campaign / Job Posting Title:", value=f"Senior {final_camp_role}")
            camp_exp = st.selectbox("Required Experience Level:", ["0-2 years (Entry)", "3-5 years (Mid)", "6-8 years (Senior)", "9+ years (Lead / Principal)"])

        with c_col2:
            camp_skills = st.text_area("Required Skills (comma separated):", value="Python, FastAPI, Docker, PostgreSQL, Microservices, CI/CD")
            camp_desc = st.text_area("Job Description & Scope:", value=f"We are hiring a high-impact {final_camp_role} to build scalable architecture, robust APIs, and enterprise cloud applications.", height=115)

        if st.button("🚀 Create Hiring Campaign", use_container_width=True, key="btn_create_camp"):
            camp_id = str(uuid.uuid4())[:8]
            st.session_state.campaigns[camp_id] = {
                "id": camp_id,
                "role": final_camp_role,
                "title": camp_title,
                "skills": [s.strip() for s in camp_skills.split(",") if s.strip()],
                "experience": camp_exp,
                "description": camp_desc,
                "created_at": datetime.now().strftime("%Y-%m-%d %H:%M")
            }
            st.session_state.active_campaign_id = camp_id
            st.success(f"Campaign created: '{camp_title}' (ID: {camp_id})")
            st.rerun()

    # 2. Bulk Resume Upload & AI Resume Screening
    with rec_tabs[1]:
        st.subheader("📤 Bulk Resume Upload & AI Screening")
        
        if not st.session_state.campaigns:
            st.info("Please start a Hiring Campaign in Tab 1 first.")
        else:
            camp_options = {cid: f"{cdata['title']} ({cdata['role']})" for cid, cdata in st.session_state.campaigns.items()}
            sel_camp_id = st.selectbox("Select Target Campaign for Screening:", list(camp_options.keys()), format_func=lambda x: camp_options[x])
            target_campaign = st.session_state.campaigns[sel_camp_id]

            uploaded_resumes = st.file_uploader(
                "Upload Candidate Resumes (PDF, DOCX, TXT):",
                type=["pdf", "docx", "txt"],
                accept_multiple_files=True,
                key="rec_bulk_resumes"
            )

            if uploaded_resumes and st.button("⚡ Run AI Screening & Extraction", use_container_width=True):
                with st.spinner(f"Screening {len(uploaded_resumes)} resumes against {target_campaign['title']}..."):
                    req_skills = target_campaign["skills"]
                    for f in uploaded_resumes:
                        try:
                            analysis = api_analyze_resume(f)
                            cand_skills = analysis.get("skills", [])
                            matched = [s for s in req_skills if any(s.lower() in cs.lower() for cs in cand_skills)]
                            missing = [s for s in req_skills if s not in matched]

                            role_match = int(min(100, (len(matched) / max(1, len(req_skills))) * 100))
                            resume_score = int(analysis.get("resume_score", random.randint(75, 95)))
                            
                            c_record = {
                                "id": str(uuid.uuid4())[:8],
                                "campaign_id": sel_camp_id,
                                "name": analysis.get("name", f.name.replace(".pdf", "")),
                                "email": analysis.get("email", f"{f.name.split('.')[0].lower()}@gmail.com"),
                                "phone": analysis.get("phone", "+91 98765 43210"),
                                "skills": cand_skills,
                                "matched_skills": matched,
                                "missing_skills": missing,
                                "experience": analysis.get("experience", "2+ Years"),
                            ...
                            c_record = {
                                ...
                                "campaign_id": sel_camp_id,
                                "name": name_clean,
                                "email": f"{name_clean.replace(' ', '.').lower()}@gmail.com",
                                "phone": "+91 98123 45678",
                                "skills": req_skills + ["Git", "SQL"],
                                "matched_skills": matched,
                                "missing_skills": missing,
                                "experience": "3 Years",
                                "resume_score": random.randint(78, 94),
                                "role_match": random.randint(75, 96),
                                "stage": "SCREENED",
                                "shortlisted": False,
                                "assessment_score": None,
                                "interview_notes": ""
                ...
                for cand in st.session_state.recruiter_candidates:
                    ...
                    <div class="panel" style="margin-bottom:10px;">
                        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap;">
                            <div>
                                <h3 style="margin:0; color:#38bdf8;">{cand['name']}</h3>
                                <p style="margin:2px 0 6px 0; color:#cbd5e1; font-size:0.88rem;">
                                    📧 {cand['email']} &nbsp;|&nbsp; 📱 {cand['phone']} &nbsp;|&nbsp; ⏳ {cand['experience']} &nbsp;|&nbsp; Stage: <b>{cand['stage']}</b>
                                </p>
                            </div>
                            <div style="display:flex; gap:10px;">
                                <span class="tag-bubble tag-cyan">Resume: {cand['resume_score']}%</span>
                                <span class="tag-bubble tag-emerald">Role Match: {cand['role_match']}%</span>
                            </div>
                        </div>
                        <div style="font-size:0.85rem; margin-top:4px;">
                            <span style="color:#4ade80;"><b>✓ Matched Skills:</b> {', '.join(cand['matched_skills']) if cand['matched_skills'] else 'None'}</span><br>
                            <span style="color:#c084fc;"><b>• Missing Skills:</b> {', '.join(cand['missing_skills']) if cand['missing_skills'] else 'None'}</span>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    # 3. Shortlisting
    with rec_tabs[2]:
        st.subheader("🏆 Shortlist Top Candidates")
        if not st.session_state.recruiter_candidates:
            st.info("No candidates screened yet. Upload resumes in Tab 2.")
        else:
            total_cand = len(st.session_state.recruiter_candidates)
            c_short1, c_short2 = st.columns([1, 2])
            with c_short1:
                shortlist_target = st.number_input("How many candidates do you want to shortlist?", min_value=1, max_value=max(1, total_cand), value=min(5, total_cand))
                if st.button("⚡ AI Auto-Shortlist", use_container_width=True):
                    sorted_pool = sorted(st.session_state.recruiter_candidates, key=lambda x: (x["role_match"], x["resume_score"]), reverse=True)
                    for idx, c in enumerate(sorted_pool):
                        c["shortlisted"] = (idx < shortlist_target)
                        if c["shortlisted"]:
                            c["stage"] = "SHORTLISTED"
                    st.success(f"Shortlisted top {shortlist_target} candidates!")
                    st.rerun()

            with c_short2:
                st.markdown("#### 🎯 Active Shortlist Review")
                for c in st.session_state.recruiter_candidates:
                    col_chk1, col_chk2 = st.columns([0.1, 0.9])
                    with col_chk1:
                        is_short = st.checkbox("", value=c["shortlisted"], key=f"chk_short_{c['id']}")
                        c["shortlisted"] = is_short
                        if is_short and c["stage"] == "SCREENED":
                            c["stage"] = "SHORTLISTED"
                    with col_chk2:
                        st.markdown(f"**{c['name']}** — {c['role_match']}% Role Match ({c['email']})")

    # 4. Generate Role-Specific Assessment
    with rec_tabs[3]:
        st.subheader("📝 Generate Role-Specific Assessment Blueprint")
        st.caption("Configure custom examination parameters for the hiring campaign.")

        ass_role = st.selectbox("Select Assessment Role Blueprint:", list(ROLE_EXAM_BLUEPRINTS.keys()), key="rec_ass_role_sel")
        ass_q_count = st.select_slider("Question Count:", options=[10, 20, 30, 40, 50], value=50)
        ass_diff = st.select_slider("Difficulty Tier:", options=["Junior", "Intermediate", "Senior", "Lead Architect"], value="Senior")

        if st.button("🔨 Build Assessment Template", use_container_width=True):
            bp = get_exam_blueprint(ass_role)
            st.session_state.current_rec_blueprint = {
                "role": ass_role,
                "questions_count": ass_q_count,
                "difficulty": ass_diff,
                "sections": bp
            }
            st.success("Assessment Blueprint built successfully!")

        if "current_rec_blueprint" in st.session_state:
            bp_data = st.session_state.current_rec_blueprint
            st.markdown(f"#### 📋 Blueprint Pattern for: `{bp_data['role']}`")
            for sec, count in bp_data["sections"]:
                st.markdown(f"- **{sec}**: {count} Questions")

    # 5. Generate Assessment Link & Send to Shortlisted Candidates
    with rec_tabs[4]:
        st.subheader("🔗 Generate & Dispatch Assessment Links")
        st.caption("The system automatically aggregates candidate emails from the shortlisted resume pool.")

        shortlisted_pool = [c for c in st.session_state.recruiter_candidates if c.get("shortlisted", False)]
        
        if not shortlisted_pool:
            st.warning("No candidates currently shortlisted. Please shortlist candidates in Tab 3.")
        else:
            st.markdown(f"#### Shortlisted Cohort ({len(shortlisted_pool)} Candidates)")
            emails_detected = [c["email"] for c in shortlisted_pool]
            for em in emails_detected:
                st.markdown(f"✓ `{em}`")

            portal_url = "https://careerlens-ai.streamlit.app/?workspace=Assessment+Exam"
            st.markdown(f"""
            <div class="panel" style="border-color:rgba(56,189,248,0.5);">
                <div style="font-weight:800; color:#38bdf8;">Assessment Portal Link:</div>
                <code style="font-size:1.05rem;">{portal_url}</code>
            </div>
            """, unsafe_allow_html=True)

            if st.button("📧 Dispatch Assessment Invitations to All Shortlisted", use_container_width=True):
                for c in shortlisted_pool:
                    c["stage"] = "ASSESSMENT SENT"
                log_event("ASSESSMENT_INVITATIONS_SENT", st.session_state.username, "N/A", f"Sent to {len(shortlisted_pool)} candidates")
                st.success(f"Assessment invitations successfully dispatched to {len(emails_detected)} candidate emails!")
                st.rerun()

    # 6. Recruiter Assessment Results & Detailed Answer Review
    with rec_tabs[5]:
        st.subheader("📊 Assessment Results & Forensic Answer Key")
        st.caption("Inspect exact candidate performance, question-by-question verdicts, and breakdown analysis.")

        if not st.session_state.recruiter_assessments:
            st.info("No candidates have submitted assessments yet. Assessments completed in the candidate portal appear here in real time.")
        else:
            for cand_email, res_data in st.session_state.recruiter_assessments.items():
                pct = res_data["percentage"]
                badge_c = "tag-emerald" if pct >= 75 else ("tag-cyan" if pct >= 50 else "tag-purple")
                
                st.markdown(f"""
                <div class="panel" style="border-color: rgba(56,189,248,0.4); margin-bottom:15px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap;">
                        <div>
                            <h3 style="margin:0; color:#38bdf8;">Candidate: {cand_email}</h3>
                            <p style="margin:2px 0 6px 0; color:#cbd5e1; font-size:0.9rem;">
                                Assessment: <b>{res_data['role']}</b> &nbsp;|&nbsp; Submitted: <b>{res_data['submitted_at']}</b>
                            </p>
                        </div>
                        <div style="display:flex; gap:10px;">
                            <span class="tag-bubble {badge_c}">Score: {res_data['score']} / {res_data['total']} ({pct}%)</span>
                            <span class="tag-bubble tag-emerald">Correct: {res_data['correct']}</span>
                            <span class="tag-bubble tag-rose">Incorrect: {res_data['incorrect']}</span>
                            <span class="tag-bubble tag-amber">Unanswered: {res_data['unanswered']}</span>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)

                with st.expander(f"🔍 Inspect Detailed Question-by-Question Answers for {cand_email}"):
                    for item in res_data.get("details", []):
                        st_color = "#4ade80" if item["status"] == "Correct" else ("#f43f5e" if item["status"] == "Incorrect" else "#fbbf24")
                        st.markdown(f"""
                        <div style="background:rgba(15,23,42,0.6); padding:10px 14px; border-radius:8px; margin-bottom:8px; border-left: 3px solid {st_color};">
                            <div style="display:flex; justify-content:space-between; font-weight:700; color:#f4f7fb;">
                                <span>Q{item['id']}: {item['question']}</span>
                                <span style="color:{st_color};">{item['status']}</span>
                            </div>
                            <div style="font-size:0.88rem; color:#cbd5e1; margin-top:4px;">
                                • <b>Candidate Answer:</b> {item['candidate_answer']}<br>
                                • <b>Correct Answer:</b> <span style="color:#4ade80;">{item['correct_answer']}</span>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)

    # 7. Candidate Hiring Pipeline & Interview Management
    with rec_tabs[6]:
        st.subheader("🏅 End-to-End Candidate Hiring Pipeline & Interviews")
        st.caption("Move talent across candidate hiring stages, schedule interviews, record feedback, and issue final hiring decisions.")

        if not st.session_state.recruiter_candidates:
            st.info("No candidates in the hiring pipeline.")
        else:
            stages_order = ["APPLIED", "SCREENED", "SHORTLISTED", "ASSESSMENT SENT", "ASSESSMENT COMPLETED", "INTERVIEW", "SELECTED", "REJECTED"]
            ...
            for c in st.session_state.recruiter_candidates:
                ...
                <div class="panel" style="margin-bottom:12px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap;">
                        <div>
                            ...
                            <p style="margin:2px 0; color:#cbd5e1; font-size:0.88rem;">
                                📧 {c['email']} &nbsp;|&nbsp; Resume Match: <b>{c['role_match']}%</b> &nbsp;|&nbsp; 
                                Assessment Score: <b>{f"{c['assessment_score']}%" if c.get('assessment_score') is not None else "Pending"}</b>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                col_stg1, col_stg2, col_stg3 = st.columns([1.5, 2, 1])
                with col_stg1:
                    current_idx = stages_order.index(c["stage"]) if c["stage"] in stages_order else 0
                    new_stage = st.selectbox("Update Pipeline Stage:", stages_order, index=current_idx, key=f"sel_stage_{c['id']}")
                    c["stage"] = new_stage

                with col_stg2:
                    notes = st.text_input("Interviewer Feedback / Notes:", value=c.get("interview_notes", ""), key=f"txt_notes_{c['id']}")
                ...
                with col_stg3:
                    if st.button(f"✉️ Send Invite", key=f"btn_inv_cand_{c['id']}"):
                        ...
                        st.success("Interview invite drafted!")
