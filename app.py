...
def _save_public_assessment_result(owner_user_id: str, assessment: Dict[str, Any], candidate_id: str, result: Dict[str, Any]) -> None:
    ...
    """
    if not owner_user_id or not candidate_id or not result.get("token"):
        return
...
def _load_persisted_assessment_submissions(owner_user_id: str) -> List[Dict[str, Any]]:
    """Load assessment results directly from the database for recruiter reporting."""
...
def render_public_recruiter_assessment(token: str) -> None:
    ...
    if window_state == "scheduled":
        ...
        st.markdown(
            f"""<div style='text-align:center;padding:20px;border:1px solid #dbeafe;border-radius:18px;background:#eff6ff;'>
            ...
            (function() {{
              ...
              function tick() {{
                ...
                if (left <= 0) {{ try {{ window.parent.location.reload(); }} catch(e) {{ window.location.reload(); }} }}
              }}
...
# ============================================================
    ...
    if st.session_state.active_tool == "Dashboard":
        ...
        if st.session_state.get("user_id"):
            persisted = _load_persisted_assessment_submissions(st.session_state.user_id)
            if persisted:
