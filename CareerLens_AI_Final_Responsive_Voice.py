...
st.markdown(
    ...
    *{ box-sizing: border-box; }
    ...
    .voice-answer-title { color: #0f172a; font-weight: 800; font-size: .86rem; }
    .voice-answer-subtitle { color: #64748b; font-size: .75rem; margin-top: 3px; line-height: 1.45; }
    @media (max-width: 768px) {
      .voice-answer-card { padding: 12px 13px; border-radius: 13px; }
    }
...
# Final visual refinement layer: softer navigation, cleaner surfaces, responsive access screens.
st.markdown(
    """
    <style>
      :root { --cl-blue:#2563eb; --cl-indigo:#4f46e5; --cl-purple:#7c3aed; --cl-line:#e5eaf2; }
      html, body, .stApp { background:#f7f9fc !important; }
      [data-testid="stSidebar"] { background:#fbfcfe !important; border-right:1px solid #e8edf4 !important; box-shadow:10px 0 30px rgba(15,23,42,.035) !important; }
      [data-testid="stSidebar"]>div:first-child { padding:18px 14px 22px !important; }
      [data-testid="stSidebar"] .stButton>button { background:#fff !important; color:#475569 !important; border:1px solid transparent !important; border-radius:11px !important; box-shadow:none !important; transform:none !important; justify-content:flex-start !important; text-align:left !important; min-height:40px !important; padding:9px 12px !important; font-size:.82rem !important; font-weight:650 !important; }
      [data-testid="stSidebar"] .stButton>button:hover { background:#f3f6fb !important; color:#1d4ed8 !important; border-color:#e5eaf2 !important; transform:translateX(2px) !important; }
      [data-testid="stSidebar"] .stButton>button[kind="primary"] { background:linear-gradient(135deg,#2563eb,#6366f1) !important; color:#fff !important; border-color:transparent !important; box-shadow:0 7px 18px rgba(37,99,235,.18) !important; }
      [data-testid="stSidebar"] .stButton>button[kind="primary"]:hover { background:linear-gradient(135deg,#1d4ed8,#5b21b6) !important; color:#fff !important; }
      .sidebar-user-box { background:#fff !important; border:1px solid #e8edf4 !important; box-shadow:0 5px 18px rgba(15,23,42,.035) !important; }
      .access-shell { max-width:1080px; margin:14px auto 0; padding:clamp(22px,4vw,46px); background:#fff; border:1px solid #e5eaf2; border-radius:28px; box-shadow:0 20px 55px rgba(15,23,42,.07); }
      .access-top { display:flex; justify-content:space-between; align-items:center; gap:18px; margin-bottom:28px; }
      .cl-brand { display:flex; align-items:center; gap:10px; font-weight:900; color:#0f172a; letter-spacing:-.035em; }
      .cl-logo { width:34px; height:34px; border-radius:10px; display:grid; place-items:center; color:#fff; font-size:12px; font-weight:900; background:linear-gradient(135deg,#2563eb,#7c3aed); box-shadow:0 7px 18px rgba(37,99,235,.2); }
      .step-nav { display:flex; gap:8px; flex-wrap:wrap; margin-bottom:20px; }
      .step { padding:7px 11px; border:1px solid #e5eaf2; border-radius:999px; color:#94a3b8; background:#fbfcfe; font-size:.7rem; font-weight:800; }
      .step.active { color:#1d4ed8; border-color:#bfdbfe; background:#eff6ff; }
      .access-grid { display:grid; grid-template-columns:1fr .78fr; gap:32px; align-items:stretch; }
      .access-copy-panel { padding:12px 8px 12px 0; display:flex; flex-direction:column; justify-content:center; }
      .access-kicker { display:inline-flex; width:max-content; padding:7px 11px; border-radius:999px; background:#eff6ff; color:#1d4ed8; border:1px solid #dbeafe; font-size:.68rem; font-weight:850; letter-spacing:.08em; }
      .access-title { font-size:clamp(2rem,4vw,3.2rem); line-height:1.05; font-weight:900; letter-spacing:-.055em; color:#0f172a; margin:16px 0 12px; }
      .access-title span { background:linear-gradient(135deg,#2563eb,#7c3aed); -webkit-background-clip:text; background-clip:text; color:transparent; }
      .access-copy { color:#64748b; font-size:.94rem; line-height:1.75; max-width:560px; }
      .access-benefits { margin-top:22px; display:grid; gap:10px; }
      .access-benefit { display:flex; gap:10px; align-items:center; color:#475569; font-size:.8rem; font-weight:650; }
      .benefit-dot { width:8px; height:8px; border-radius:50%; background:linear-gradient(135deg,#2563eb,#7c3aed); box-shadow:0 0 0 5px #eef2ff; flex:0 0 auto; }
      .auth-panel { border:1px solid #e5eaf2; border-radius:22px; padding:22px; background:#fbfcfe; }
      .guest-panel { margin-top:16px; padding:15px 16px; border-radius:16px; border:1px solid #dbeafe; background:linear-gradient(135deg,#f8fbff,#faf8ff); }
      .guest-title { font-weight:850; color:#0f172a; font-size:.9rem; }
      .guest-copy { color:#64748b; font-size:.75rem; margin:3px 0 11px; line-height:1.55; }
      .auth-note { color:#94a3b8; font-size:.68rem; line-height:1.5; margin-top:10px; }
      .content-box,.gateway-card,.kpi-card,.tool-box-card { box-shadow:0 7px 22px rgba(15,23,42,.045) !important; }
      .gateway-card { border-radius:22px !important; }
      .tool-box-card { transition:transform .18s ease, box-shadow .18s ease, border-color .18s ease; }
      .tool-box-card:hover { transform:translateY(-3px); box-shadow:0 13px 30px rgba(15,23,42,.08) !important; border-color:#d5def0; }
      @media (max-width:900px) { .access-grid{grid-template-columns:1fr;gap:20px;} .access-copy-panel{padding-right:0;} }
      @media (max-width:600px) { .access-shell{padding:18px;border-radius:20px;} .access-title{font-size:2rem;} .auth-panel{padding:16px;} }
    </style>
    """,
    unsafe_allow_html=True,
)
...
def _get_openai_api_key() -> str:
    """Read the transcription key from environment or Streamlit secrets."""
    if OPENAI_API_KEY:
...
def transcribe_interview_audio(audio_file) -> tuple[bool, str]:
    """Transcribe a recorded interview answer and return editable text."""
    if audio_file is None:
        return False, "No recording was provided."

    api_key = _get_openai_api_key()
    if not api_key:
        return False, "Voice transcription is not configured. Add OPENAI_API_KEY to the app environment or Streamlit secrets."

    try:
        audio_bytes = audio_file.getvalue()
        if not audio_bytes:
            return False, "The recording is empty. Please record your answer again."

        filename = getattr(audio_file, "name", "interview_answer.wav") or "interview_answer.wav"
        ...
        if response.status_code != 200:
            try:
                detail = response.json().get("error", {}).get("message", "Transcription request failed.")
            except Exception:
                detail = f"Transcription request failed with status {response.status_code}."
            return False, str(detail)

        transcript = str(response.json().get("text", "")).strip()
        if not transcript:
            return False, "No speech was detected in the recording. Please try again."
        return True, transcript
    except requests.RequestException as exc:
        return False, f"Voice transcription could not connect to the transcription service: {exc}"
    except Exception as exc:
        return False, f"Voice transcription failed: {exc}"
...
defaults = {
    ...
    "interview_voice_transcript": "",
    "interview_voice_audio_hash": "",
    "interview_voice_error": "",
...
if not st.session_state.is_logged_in:
    ...
    if _access_action:
        try:
            ...
            pass
        if _access_action in {"signin", "register"}:
            st.session_state.access_screen = True
        elif _access_action == "guest":
            st.session_state.user_id = ""
            st.session_state.username = "Guest Explorer"
            st.session_state.is_logged_in = True
            st.session_state.selected_gateway = False
            st.session_state.access_screen = False
    ...
    if not st.session_state.access_screen:
        st.markdown("""
        <div class="landing-frame">
          <div class="landing-content">
            <div class="landing-nav">
              <div class="cl-brand"><div class="cl-logo">CL</div><div class="brand">Career<span class="brand-accent">Lens</span> AI</div></div>
              <div class="landing-links"><span class="landing-link">Features</span><span class="landing-link">For Job Seekers</span><span class="landing-link">For Recruiters</span><span class="landing-link">Career Intelligence</span></div>
            </div>
            <div class="landing-kicker">AI-POWERED CAREER INTELLIGENCE</div>
            <div class="landing-title">Your Career.<br><span class="landing-gradient">Intelligently Mapped.</span></div>
            <div class="landing-copy">Discover opportunities, understand your strengths, prepare for interviews, and make better career decisions with one focused intelligence platform.</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
        f1, f2, f3 = st.columns(3, gap="medium")
        features=[("01","AI-Powered Insights","Turn your resume and career data into practical decisions."),("02","Personalized Roadmaps","Build a clear path from your current skills to your target role."),("03","Trusted & Secure","Keep your career workspace focused, private, and easy to control.")]
        for col,(num,title,desc) in zip((f1,f2,f3),features):
            with col:
                st.markdown(f'''<div class="landing-feature"><div class="semantic-icon">{num}</div><div class="landing-feature-title">{title}</div><div class="landing-feature-text">{desc}</div></div>''',unsafe_allow_html=True)
        st.markdown("""
        <div style="margin:24px 0 8px;padding:20px 22px;border:1px solid #e5eaf2;border-radius:20px;background:#fff;box-shadow:0 8px 25px rgba(15,23,42,.04);">
          <div style="font-size:1rem;font-weight:900;color:#0f172a;">One workspace for your career journey</div>
          <div style="font-size:.8rem;color:#64748b;margin-top:5px;line-height:1.6;">Resume intelligence, job matching, assessments, mock interviews, salary insights, roadmaps, and recruiter workflows—connected in one experience.</div>
        </div>
        """,unsafe_allow_html=True)
        cta1,cta2=st.columns(2,gap="small")
        with cta1:
            if st.button("Get Started",use_container_width=True,type="primary",key="landing_get_started"):
                st.session_state.access_screen=True; st.rerun()
        with cta2:
            if st.button("Explore the Platform",use_container_width=True,key="landing_explore"):
                st.session_state.access_screen=True; st.rerun()
        st.stop()

    st.markdown("""
    <div class="access-shell">
      <div class="access-top"><div class="cl-brand"><div class="cl-logo">CL</div><div style="font-size:1.1rem;">Career<span class="brand-accent">Lens</span> AI</div></div><div style="font-size:.72rem;color:#64748b;font-weight:700;">Step 2 of 3 · Account Access</div></div>
      <div class="step-nav"><span class="step">1 · Home</span><span class="step active">2 · Access Account</span><span class="step">3 · Choose Workspace</span></div>
      <div class="access-grid">
        <div class="access-copy-panel">
          <div class="access-kicker">CAREERLENS AI</div>
          <div class="access-title">Your next career move<br><span>starts here.</span></div>
          <div class="access-copy">Sign in to continue your career workspace, create a new account, or explore the platform instantly without creating one.</div>
          <div class="access-benefits"><div class="access-benefit"><span class="benefit-dot"></span>Personalized career intelligence</div><div class="access-benefit"><span class="benefit-dot"></span>Resume, job-match and interview tools</div><div class="access-benefit"><span class="benefit-dot"></span>Separate Job Seeker and Recruiter workspaces</div></div>
        </div>
    ...
    with tab_signin:
        st.markdown("<div style='font-size:.82rem;color:#64748b;margin:2px 0 12px;'>Continue to your CareerLens workspace.</div>",unsafe_allow_html=True)
        with st.form("access_signin_form",clear_on_submit=False):
            u=st.text_input("Username or Email",key="access_sign_u")
            p=st.text_input("Password",type="password",key="access_sign_p")
            submitted=st.form_submit_button("Sign In",use_container_width=True,type="primary")
        if submitted:
            if not u or not p: st.warning("Please fill in both fields.")
            else:
                account=_db_user(u)
                if account and password_matches(account[3],p):
                    st.session_state.user_id=account[0]; st.session_state.username=account[2]; st.session_state.is_logged_in=True; st.session_state.selected_gateway=False; st.session_state.access_screen=False
                    saved=_db_load_state(st.session_state.user_id)
                    for key,value in saved.items(): st.session_state[key]=value
                    st.session_state.recruiter_data=_db_load_recruiter_state(st.session_state.user_id)
                    st.session_state.recruiter_candidates=st.session_state.recruiter_data.get("candidates",[])
                    ...
                    log_event("LOGIN",st.session_state.username,"N/A","User Login"); st.rerun()
                elif ADMIN_PIN and u.lower()=="admin" and p==ADMIN_PIN:
                    st.session_state.user_id="admin"; st.session_state.username="Administrator"; st.session_state.is_logged_in=True; st.session_state.selected_gateway=True; st.session_state.access_screen=False; st.session_state.active_workspace="Recruiter Workspace"; st.rerun()
                else: st.error("Invalid credentials. Please check your details or create an account.")
    with tab_register:
        st.markdown("<div style='font-size:.82rem;color:#64748b;margin:2px 0 12px;'>Create your personal CareerLens workspace.</div>",unsafe_allow_html=True)
        with st.form("access_register_form",clear_on_submit=False):
            reg_n=st.text_input("Full Name",key="access_reg_n")
            reg_u=st.text_input("Username / Email",key="access_reg_u")
            reg_p=st.text_input("Create Password",type="password",key="access_reg_p")
            submitted=st.form_submit_button("Create Account",use_container_width=True,type="primary")
        if submitted:
            if not reg_u or not reg_p: st.warning("Username and password are required.")
            elif len(reg_p)<6: st.warning("Use a password with at least 6 characters.")
            elif _db_user(reg_u): st.error("That username or email is already registered. Please sign in.")
            else:
                try:
                    uid=_db_create_user(reg_u,reg_n,hash_password(reg_p))
                    st.session_state.user_id=uid; st.session_state.username=reg_n.strip() if reg_n.strip() else reg_u.split("@")[0].capitalize(); st.session_state.is_logged_in=True; st.session_state.selected_gateway=False; st.session_state.access_screen=False
                    ...
                    _db_save_state(uid,{"username":st.session_state.username,"resume_text":"","resume_analysis":None,"job_match_result":None,"resume_builder":{}})
                    log_event("REGISTER",st.session_state.username,"N/A",f"Registered: {reg_u}"); st.rerun()
                except sqlite3.IntegrityError: st.error("That username or email is already registered. Please sign in.")
    st.markdown("""
          <div class="guest-panel"><div class="guest-title">Continue as Guest</div><div class="guest-copy">Explore the CareerLens experience without creating an account. Your guest session is temporary and does not save personal workspace data.</div></div>
    """,unsafe_allow_html=True)
    if st.button("Continue as Guest",use_container_width=True,key="access_guest",type="primary"):
        st.session_state.user_id=""; st.session_state.username="Guest Explorer"; st.session_state.is_logged_in=True; st.session_state.selected_gateway=False; st.session_state.access_screen=False
        st.session_state.resume_text=""; st.session_state.resume_analysis=None; st.session_state.job_match_result=None; st.session_state.resume_builder={}
        st.session_state.recruiter_data={"campaign":None,"candidates":[],"assessments":[],"submissions":[]}; st.session_state.recruiter_candidates=[]; st.session_state.recruiter_assessment_submissions={}
        log_event("GUEST_ACCESS","Guest","N/A","Guest entry"); st.rerun()
    st.markdown("<div class='auth-note'>Guest access is designed for exploration and does not persist personal data.</div>",unsafe_allow_html=True)
    st.markdown("</div></div></div>",unsafe_allow_html=True)
    if st.button("Back to Home",key="access_back_home",use_container_width=True):
        st.session_state.access_screen=False; st.rerun()
    st.stop()


# ============================================================
# 3. WORKSPACE GATEWAY PORTAL
...
if not st.session_state.selected_gateway:
    st.markdown(
        ...
        <div class="header-banner">
          <div>
            <div class="header-title">Welcome to your workspace, {html.escape(st.session_state.username)} </div>
    ...
    with g1:
        ...
        <div class="gateway-card">
          <div style="display:flex;align-items:center;gap:14px;margin-bottom:14px">
            <div class="role-icon" style="background:linear-gradient(135deg,#2563eb,#06b6d4);">JS</div>
    ...
    with g2:
        ...
        <div class="gateway-card">
          <div style="display:flex;align-items:center;gap:14px;margin-bottom:14px">
            <div class="role-icon" style="background:linear-gradient(135deg,#7c3aed,#ec4899);">TA</div>
...
with st.sidebar:
    st.markdown(
        ...
        <div class="sidebar-brand-box">
            <div class="cl-logo" style="width:34px;height:34px;border-radius:10px;">CL</div>
            <div>
    ...
    st.markdown(
        ...
        <div class="sidebar-user-box">
            <div style="display:flex; align-items:center; gap:10px;">
                <div style="width:36px; height:36px; border-radius:50%; background:linear-gradient(135deg, #2563eb, #7c3aed); color:#fff; display:flex; align-items:center; justify-content:center; font-weight:800;">U</div>
                <div>
    ...
    if st.button("Logout", key="sb_logout_btn", use_container_width=True):
        ...
        if uid:
            try:
                ...
                pass
        for key in ["is_logged_in", "selected_gateway", "access_screen", "user_id"]:
...
if st.session_state.active_workspace == "Job Seeker Workspace":
    ...
    if st.session_state.active_tool == "Dashboard":
        ...
        with c6:
            ...
            if st.button("Career Roadmap", key="card_c6_btn", use_container_width=True):
                ...
                st.rerun()
        with c7:
            st.markdown("""<div class="tool-box-card"><div class="tool-icon-circle" style="background:#fef2f2; color:#ef4444;">Secure</div><div class="tool-title">Job Detection</div><div class="tool-desc">Real-time scam and fake job offer detection.</div></div>""", unsafe_allow_html=True)
        ...
        if not st.session_state.interview_active and not st.session_state.interview_completed:
            ...
            if st.button(" Start Interview", use_container_width=True, type="primary", key="start_mock_new"):
                ...
                if len(role) < 2:
                    ...
                    st.session_state.interview_voice_transcript = ""
                    st.session_state.interview_voice_audio_hash = ""
                    st.session_state.interview_voice_error = ""
                    st.rerun()
            ...
            st.markdown("<div class='voice-answer-card'><div class='voice-answer-title'>Answer mode</div><div class='voice-answer-subtitle'>Type your response or record your answer and turn it into an editable transcript.</div></div>", unsafe_allow_html=True)
            ...
            answer_mode = st.radio(
                ...
                key=f"mock_answer_mode_{question_no}",
            )

            if answer_mode == "Voice":
                audio_recorder = getattr(st, "audio_input", None)
                if audio_recorder is None:
                    st.warning("Voice recording requires a recent Streamlit version. Type your answer below or upgrade Streamlit.")
                    cand_response = st.text_area("Your response", height=190, key=f"ans_text_voice_fallback_{question_no}")
                else:
                    st.caption("Record your answer. Your browser will ask for microphone permission the first time.")
                    audio_file = audio_recorder("Record answer", key=f"mock_voice_{question_no}")
                    if audio_file is not None:
                        try:
                            audio_hash = hashlib.sha256(audio_file.getvalue()).hexdigest()
                        except Exception:
                            audio_hash = ""
                        if audio_hash and audio_hash != st.session_state.get("interview_voice_audio_hash", ""):
                            st.session_state.interview_voice_audio_hash = audio_hash
                            st.session_state.interview_voice_error = ""
                            with st.spinner("Transcribing your answer…"):
                                ok, result = transcribe_interview_audio(audio_file)
                            if ok:
                                st.session_state.interview_voice_transcript = result
                            else:
                                st.session_state.interview_voice_error = result

                    if st.session_state.get("interview_voice_error"):
                        st.error(st.session_state.interview_voice_error)

                    cand_response = st.text_area(
                        "Editable transcript",
                        value=st.session_state.get("interview_voice_transcript", ""),
                        height=180,
                        key=f"voice_transcript_edit_{question_no}",
                        placeholder="Your spoken answer will appear here. Edit it if needed before submitting.",
                    )
                    st.session_state.interview_voice_transcript = cand_response
            else:
                cand_response = st.text_area(
                    "Your response",
                    height=190,
                    key=f"ans_text_live_{question_no}",
    ...
    # 4. AI JOB MATCH
        ...
        if st.session_state.job_match_result:
            ...
            with t2:
                st.markdown("#### Warning:  Skills to Upgrade")
